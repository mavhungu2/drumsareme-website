/**
 * One-off migration: copies the 15 hardcoded products from
 * `src/lib/products.ts` (legacy in-repo catalog) into Firestore
 * `products/{id}` so the admin can manage them at runtime.
 *
 * Run from functions/:
 *   GCLOUD_PROJECT=drumsareme-website npx ts-node src/scripts/seedProducts.ts
 *
 * Idempotent: writes use `set(..., {merge: true})` so re-running won't lose
 * any admin edits made after the initial seed (it only repopulates fields).
 */
...
async function main() {
  ...
  for (let i = 0; i < SEED.length; i += 1) {
    const p = SEED[i];
    const data: Omit<Product, "updatedAt"> & {
    ...
    console.log(
      `  ${i.toString().padStart(2)} ${p.id.padEnd(18)} ${p.size.padEnd(6)} ${p.color}`,
    );
  }
  console.log("Done.");
}
